...
- `assets/css/main.css` owns the left sticky desktop panel, sidebar stretch behavior, collapsed rail state, active-link styling, and mobile collapsible panel.
- `assets/js/content-navigator.js` owns the desktop show/hide toggle, expanded-on-load state, desktop scroll-synced active tracking, active-link reveal inside the sticky navigator, and mobile close-on-select behavior.

Keep the navigator posts-only and dependency-free. Post detail pages use the `article-layout-post` width contract in `assets/css/main.css`: the article targets a centered `1020px` content column where viewport space allows. The desktop navigator should stay on the left side, load expanded even when older localStorage values exist, expand as an in-flow sidebar that pushes the article to the right, collapse to a narrow rail instead of a modal/overlay, and use matching collapsed rails so the article recenters when the navigator is hidden. Keep the desktop navigator's sidebar stretched to the article height so sticky positioning continues while reading the full post. Keep `aria-expanded`, `aria-controls`, labels, and titles synced with state. Active desktop tracking should update while scrolling, after hash changes, and after navigator link clicks, and the active desktop link should stay visible by scrolling only the navigator container, not the main page.

When changing navigator behavior, verify post-only rendering, left-side desktop placement, full-post sticky behavior, expanded sidebar push, centered collapsed article, expanded-on-refresh behavior, scroll-synced active highlighting, active-link reveal inside an overflowing desktop navigator, correct anchor scrolling with the sticky header, mobile fit, and light/dark/system theme readability.
...
- Content navigator changes preserve posts-only rendering, left-side desktop placement, full-post sticky behavior, expanded-on-load behavior, expanded-sidebar article push, centered collapsed article, active-link reveal inside the desktop navigator, and scroll-synced active tracking.
